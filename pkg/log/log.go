...
var (
	...
	defaultPbJsonMarshaler = jsonpb.Marshaler{}
	asynccore              *asyncCore
...
func Pb(key string, pbMsg proto.Message) zap.Field {
	...
	err := defaultPbJsonMarshaler.Marshal(&buf, pbMsg)
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "log.Pb marshal to json err: %v, pbMsg: %s", err, pbMsg.String())
	}
	return zap.Any(key, json.RawMessage(buf.Bytes()))
}
